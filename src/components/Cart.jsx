...
const Cart = () => {
  const { savatItems, savatdanOlib, savatgaQoshish, soniniKamaytirish, savatniTozalash, jamiNarx } = useCart();
  const { user } = useAuth();
  ...
  return (
    <div className="p-4 lg:p-6">
      <h2 className="text-xl lg:text-2xl font-bold text-gray-900 mb-6">Savat</h2>
      ...
      {savatItems.length === 0 ? (
        <div className="text-center py-12">
          ...
          <p className="text-gray-400 text-sm mt-2">Mahsulotlarni savatga qo'shing</p>
        </div>
      ) : (
        <div className="space-y-6">
            ...
            {savatItems.map((item) => (
              <div key={item.id} className="flex flex-col sm:flex-row items-start sm:items-center p-4 border-b last:border-b-0 gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <h3 className="font-semibold text-gray-900 truncate">{item.nomi}</h3>
                  <p className="text-blue-600 font-medium">{formatNarx(item.narx)}</p>
                </div>
                <div className="flex items-center space-x-3 flex-shrink-0">
                  ...
                  <span className="font-semibold min-w-[2rem] text-center">{item.soni}</span>
                  <button
          ...
          <div className="bg-white rounded-lg shadow-md p-6">
            <div className="flex justify-between items-center mb-4">
              ...
              <span className="text-2xl font-bold text-blue-600">{formatNarx(jamiNarx)}</span>
            </div>
            <button
